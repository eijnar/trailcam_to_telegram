...
PROCESSED_DIRECTORY = os.getenv('PROCESSED_DIRECTORY', '/home/camera/')
        ...
        try:
            if file_type == 'photo':
                with open(file_path, 'rb') as photo:
                    self.bot.send_photo(chat_id=TELEGRAM_CHAT_ID, photo=photo, caption=f"New upload: {filename}")
            elif file_type == 'video':
                with open(file_path, 'rb') as video:
                    self.bot.send_video(chat_id=TELEGRAM_CHAT_ID, video=video, caption=f"New upload: {filename}")
